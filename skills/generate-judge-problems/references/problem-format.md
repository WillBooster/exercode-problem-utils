...
- Do not duplicate `.in` contents across cases (the validator compares stdin only, not `.fin/` contents), and never produce an empty `.out` (omit the file instead when stdout is not compared).
