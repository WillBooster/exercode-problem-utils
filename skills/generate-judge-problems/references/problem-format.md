...
- Do not duplicate `.in` contents across cases (the validator compares stdin only, not `.fin/` contents), and never produce an empty `.out` (omit the file instead when stdout is not compared; the validator warns about one). A `.out` must not exceed 50,000 characters after trimming trailing whitespace: the stdio judge rejects a run that prints more than that.

## Templates

Starter code under `templates/<languageId>/` (or `templates/_default/`; files placed directly under `templates/` also count as `_default`) must be incomplete: it must not solve the problem, so no template directory may contain, unchanged, every file a model answer is judged by (its non-empty source files of a supported language for a standard problem; every non-empty file for a custom `judge.ts`; notes next to the answer are not compared). When a problem provides helper modules to learners (e.g. a module to import), ship them via templates; such helper files are expected to be identical in the templates and in the model answers.
