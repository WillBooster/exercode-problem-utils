...
- `stdioDebugPreset` copies the answer directory to a temporary directory and builds and runs it there together with `_shared.fin/` and the first example case's `.fin/` (the case's files win; hidden cases' inputs are never handed to learner code), so the answer directory is left untouched; files the program writes are reported only through `requiredOutputFilePaths`.
