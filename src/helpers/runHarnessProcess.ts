 ...
 * `spawnWithLimits` run in sessions of their own, out of that kill's reach; each wrapper's process
 * group is bounded by its own GNU `timeout` instead, while a descendant that moved to yet another
 * session may survive without stalling the harness.
 */
