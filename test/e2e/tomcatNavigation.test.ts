...
test('course navigation accepts the endpoint path and reports a wrong destination', { timeout: 30_000 }, async () => {
  await using server = http.createServer((_request, response) => response.end('<h1>Course page</h1>'));
  ...
  try {
    ...
    await Promise.all([verifyTomcatPath(page, '/result.jsp'), page.goto(`${baseUrl}${expectedPath}?answer=42`)]);
